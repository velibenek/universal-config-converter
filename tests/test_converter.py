...
XML_SAMPLE_DATA_FROM_STR = {
    'config': {
        'database': {'@host': 'localhost', 'port': '5432', 'user': 'xml_user', 'enabled': 'true'},
...
def test_unsupported_source_format_error(temp_files):
    ...
    with pytest.raises(ValueError, match="Unsupported source format: unknown"):
        load_config(temp_files["json_in"], "unknown") # File content doesn't matter here
...
def test_json_to_ini(temp_files):
    ...
    loaded_data = {section: dict(config.items(section)) for section in config.sections()}
